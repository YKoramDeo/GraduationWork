...
public class Serializer
{
    ...
    private MemoryStream mBuffer = null;
    ...
    public Serializer()
    {
        ...
        mEndianness = (conv[0] == 1) ? Endianness.LittleEndian : Endianness.BigEndian;
    }
    ...
    public bool SetDeserializedData(byte[] data)
    {
        ...
        {
            // 디시리얼라이즈할 데이터를 설정합니다.
            mBuffer.Write(data, 0, data.Length);
        }
        catch
        {
            return false;
    ...
    protected bool Serialize(byte[] element, int length)
    {
        ...
        // 여기서 원래대로 돌아가게 합니다.
        if (mEndianness == Endianness.BigEndian)
        {
    ...
    protected bool Deserialize(ref byte element)
    {
        ...
        if (ret == true)
        {
            // 읽어낸 값을 변환합니다. byte이기 때문에 변환을 거치지 않아도 된다.
            element = data[0];
            return true;
        }
